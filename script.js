  ...
  if (contactForm) {
    ...
    const getErrorMessage = function (field, rule) {
      ...
      const messages = {
        ...
        minLength: 'Please enter at least ' + field.dataset.minLength + ' characters.',
    ...
    const validateField = function (field) {
      ...
      const dataType = field.dataset.validate;

      if (isRequired) {
        if (field.type === 'checkbox' && !field.checked) {
          showError(field, getErrorMessage(field, 'required'));
          return false;
        } else if (field.type !== 'checkbox' && !validators.required(String(value))) {
          showError(field, getErrorMessage(field, 'required'));
          return false;
        }
      }
      ...
      if (fieldType === 'email' || dataType === 'email') {
        if (!validators.email(String(value))) {
          showError(field, getErrorMessage(field, 'email'));
          return false;
        }
      }

      if (fieldType === 'tel' || dataType === 'phone') {
        if (!validators.phone(String(value))) {
      ...
      if (field.dataset.minLength) {
        if (!validators.minLength(String(value), field.dataset.minLength)) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "יערת דבש משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'f7921984-1101-4c32-9b0b-0e869b25ea46',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'f7921984-1101-4c32-9b0b-0e869b25ea46';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f7921984-1101-4c32-9b0b-0e869b25ea46';
